...
Persoana e clasa de baza abstracta contine nume, prenume, varsta si un contor static numarTotal. Nu poate fi folosita direct pentru ca are functii virtuale pure.
...
Director are departament si aniExperienta. Daca are minim 5 ani experienta poate aproba burse.

## Design Patterns

Factory - FabricaPersoana are functii statice: creeazaStudent(), creeazaProfesor() etc care returneaza Persoana*. In loc de new Student direct in main totul trece prin fabrica.

Singleton - Catalog are constructorul privat si getInstance() care returneaza mereu aceeasi lucru. Nu poti crea doua cataloage orice apel la getInstance() da acelasi obiect. La sfarsit se apeleaza Catalog::distruge() ca sa se elibereze memoria.

Builder - CatalogBuilder construieste catalogul pas cu pas adica builder.adaugaStudent(p1).adaugaProfesor(p2).build(). Fiecare functie returneaza referinta la builder ca sa se poata pune unul dupa altul apelurile la final build() da pointerul la catalog
